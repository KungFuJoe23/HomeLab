...
Connect-VIServer vcsa.local.lan -User 'administrator@vsphere.local' -Password 'B0Cadence&'

$Report = Get-VM | Get-Snapshot | Select-Object VM,Name,Description,@{Label="Size";Expression={"{0:N2} GB" -f ($_.SizeGB)}},Created
If (-not $Report)
...
$Report = $Report | Select-Object VM,Name,Description,Size,Created | ConvertTo-Html -Head $Header -PreContent "<p><h2>Snapshot Report - $VIServer</h2></p><br>" | Set-AlternatingRows -CSSEvenClass even -CSSOddClass odd | Out-File c:\scripts\ps\vmware\SnapshotReport.htm | Invoke-Item c:\scripts\ps\vmware\SnapshotReport.htm
